...
if ($gclient->getAccessToken()) {
  include 'config.php';
  $pdo = new PDO('mysql:host='.'localhost'.';dbname='.'cigo_db', 'root', '');
  ...
  $sql = $pdo->prepare("SELECT id, name FROM users WHERE email=:a");
  ...
  if(empty($user)){ // Jika User dengan email tersebut belum ada
    ...
    $sql = $pdo->prepare("INSERT INTO users(id, name, email) VALUES(:username,:nama,:email)");
